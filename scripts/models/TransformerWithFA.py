...
def make_model(N=2, d_model=128, d_ff=512, h=8, dropout=0.1):
...
class ColorBBoxTransformerWithFA (TripletBase) : 
    def __init__(self, opts):  
        super(ColorBBoxTransformerWithFA, self).__init__(opts)
        fourier_dim = 5
        self.embed = nn.Sequential(
            FourierAmplification(fourier_dim),
            nn.Linear(12 * (2 * fourier_dim + 1), opts.embedding_size)
        )
    ...
    def nodeFeatures(cls, t, ps, opts) : 
        data_ = super(ColorBBoxTransformerWithFA, cls).nodeFeatures(t, ps, opts)
