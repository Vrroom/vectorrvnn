...
class TransformerPlus (TripletBase) : 
    def __init__(self, opts):  
        super(TransformerPlus, self).__init__(opts)
        fourier_dim = 2
        self.embed = nn.Sequential(
            FourierAmplification(fourier_dim),
            nn.Linear(13 * (2 * fourier_dim + 1), opts.embedding_size)
        )
    ...
    def embedding (self, node, **kwargs) : 
        ...
        combined = torch.cat((bboxes, stroke, fill, closed), dim=2) # [B, L, 13]
    ...
    def nodeFeatures(cls, t, ps, opts) : 
        data_ = super(TransformerPlus, cls).nodeFeatures(t, ps, opts)
