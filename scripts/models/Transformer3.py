...
class FourierColorBBoxTransformer (TripletBase) : 
    def __init__(self, opts):  
        super(FourierColorBBoxTransformer, self).__init__(opts)
        self.embed = nn.Linear(32, opts.embedding_size)
    ...
    def embedding (self, node, **kwargs) : 
        ...
        fourier = node['fourier'] # [B, L, 20] 
        combined = torch.cat((bboxes, stroke, fill, fourier), dim=2) # [B, L, 32]
    ...
    def nodeFeatures(cls, t, ps, opts) : 
        data_ = super(FourierColorBBoxTransformer, cls).nodeFeatures(t, ps, opts)
        ...
        fourier = [fourier_descriptor(t, i).tolist() for i in ps]
        fourier = fourier + [[-1] * 20 for _ in range(max_len - len(ps))] 
        data = dict()
        data['fourier'] = torch.tensor(fourier).float()
